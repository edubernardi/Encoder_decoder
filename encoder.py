    ...
    if function.lower() == "encode":
        ...
        if len(sys.argv) == 5:
            ...
            if method.lower() == "golomb" or method.lower() == "1":
                ...
                print("Started encoding", input_file, "into", output_file_name,
                      "using Elias-Gamma method" + current_time)
            ...
            print("Noise control file created: " + sys.argv[3] + ".ecc" + " - Execution time: " + str(
                time.process_time()) + "s")
        else:
            app_help()
    elif function.lower() == "decode":
        if input_file.count(".") < 1 or (
                input_file.split('.')[len(input_file.split('.')) - 1] != "ecc" and input_file.split('.')[
            len(input_file.split('.')) - 1] != "cod"):
            ...
            if input_file.split('.')[len(input_file.split('.')) - 1] == "cod":
                ...
                has_ecc = True
            print("Started decoding", input_file, "into",
                  output_file_name + " - Execution time: " + str(time.process_time()) + "s")
            if has_ecc:
